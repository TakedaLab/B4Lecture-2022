...
def viterbi_plus(self, n_out, size_out, output):
        """ 
        Viterbi algorithm
...
def main(args):
    ...
    # 確率の計算
    models = []
    prob_forward = np.zeros((modelsize, n_out))
    # PSI = np.zeros((modelsize, n_out, m, k))
    prob_viterbi = np.zeros((modelsize, n_out))
    for i in range(modelsize):
        models.append(HMM(k, m, a[i], b[i], pi[i]))
        prob_forward[i] = models[i].forward(n_out, size_out, output)
        prob_viterbi[i] = models[i].viterbi(n_out, size_out, output)
    prob_forward = prob_forward.T
    prob_viterbi = prob_viterbi.T
    # 生成元のモデルを予測
