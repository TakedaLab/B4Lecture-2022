...
class Sound_Data(Dataset):
    def __init__(self, meta_data, phase, maxLength=None) -> None:
        ...
        # 最長音声の長さ
        if phase == "train":
            self.maxLength = np.max([len(self.data[i])
        ...
        for i in tqdm(range(self.len)):
            self.data[i] = np.pad(
                self.data[i], (0, self.maxLength - len(self.data[i])))
...
def train(dataloader, model, loss_fn, optimizer, device):
    ...
    for (inputs, labels) in dataloader:
        inputs, labels = inputs[:, np.newaxis, :, :].to(
...
def main():
    ...
    for t in range(epochs):
        print(f"Epoch {t+1}\n------------------------")
        train_corrects[t] = train(
            train_loader, model, loss_fn, optimizer, device)
        test_corrects[t] = test(test_loader, model, loss_fn, device)
    print("Done!")
